...
Foram aferidos o tempo médio de resposta (figura~\ref{Fig:grafico_tempos_de_resposta}), o número de requisições atendidas por segundo, chamado de \textit{throughput} (figura~\ref{Fig:grafico_throughput}), e o número de requisições atendidas em uma simulação de um usuário requisitando cotações repetidas vezes às duas soluções propostas (atualização de dados sob demanda ao web service e obtenção de dados de um banco de dados atualizado independentemente de demanda). A simulação durou sessenta segundos e foi realizada utilizando a ferramenta Pylot \cite{ref-pylot}.
